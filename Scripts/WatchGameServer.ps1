...
function CreateSymbolicLinks {
    ...
    # List of directories and files to link
    $itemsToLink = @(
        "Config.json",
        "ClientData", 
        "Configurations",
    ...
    foreach ($item in $itemsToLink) {
        ...
        # Remove existing file or directory at target path if it's not a correct symlink
...
# Function to find the game process spawned by dotnet watch
function Find-GameProcess {
    $processes = Get-Process -Name "AAEmu.Game" -ErrorAction SilentlyContinue
...
function Start-DotnetWatch {
    try {
        ...
        Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Waiting for game process to start..." -ForegroundColor Cyan
        Start-Sleep -Seconds 10
        
        # Find the game process
        $global:gameProcess = Find-GameProcess
        if ($global:gameProcess -ne $null) {
            Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Game process found with PID: $($global:gameProcess.Id)" -ForegroundColor Green
        } else {
            Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Warning: Game process not found after waiting" -ForegroundColor Yellow
...
# Add this function before the Cleanup function
function Send-CtrlC {
    param (
...
function Cleanup {
    ...
    # Try to gracefully terminate game process first
    $gameProcess = Find-GameProcess
    if ($gameProcess -ne $null -and !$gameProcess.HasExited) {
        Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Attempting graceful shutdown of game process..." -ForegroundColor Yellow
        $gracefulExit = Send-CtrlC -Process $gameProcess -TimeoutSeconds 15
        Start-Sleep -Seconds 15
        
        # If graceful exit failed, force termination
        if (!$gracefulExit -and !$gameProcess.HasExited) {
...
# Start the watcher
Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Starting AAEmu.Game watcher using dotnet watch with hot-reload..." -ForegroundColor Cyan
Start-DotnetWatch
...
try {
    ...
    # Monitor for game process terminations and update our reference
    while ($true) {
        Start-Sleep -Seconds 5
        
        # Check if our game process reference is still valid
        if ($global:gameProcess -ne $null -and $global:gameProcess.HasExited) {
            Write-Host "[$(Get-Date -Format 'HH:mm:ss')] Game process exited, looking for new process..." -ForegroundColor Yellow
            $global:gameProcess = Find-GameProcess
            if ($global:gameProcess -ne $null) {
                Write-Host "[$(Get-Date -Format 'HH:mm:ss')] New game process found with PID: $($global:gameProcess.Id)" -ForegroundColor Green
            }
        }
