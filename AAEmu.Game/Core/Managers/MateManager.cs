...
public class MateManager : Singleton<MateManager>
{
    ...
    public void ChangeTargetMate(GameConnection connection, uint tlId, uint objId)
    {
        ...
        _logger.Debug("ChangeTargetMate. tlId: {0}, objId: {1}, targetObjId: {2}", mateInfo.TlId, mateInfo.ObjId, objId);
    }
    ...
    public void MountMate(GameConnection connection, uint tlId, AttachPointKind attachPoint, AttachUnitReason reason)
    {
        ...
        if (mateInfo.Passengers.TryGetValue(attachPoint, out var seatInfo))
        {
            ...
            {
                _logger.Warn("MountMate. Non-owner {0} ({1}) tried to take the first seat on mount {2} ({3})",
        ...
        {
            _logger.Warn("MountMate. Player {0} ({1}) tried to take a invalid seat {4} on mount {2} ({3})",
        ...
        _logger.Debug("MountMate. mountTlId: {0}, attachPoint: {1}, reason: {2}, seats: {3}",
    ...
    public void UnMountMate(Character character, uint tlId, AttachPointKind attachPoint, AttachUnitReason reason)
    {
        ...
        if (targetObj != null)
        {
            ...
            _logger.Debug("UnMountMate. mountTlId: {0}, targetObjId: {1}, attachPoint: {2}, reason: {3}", mateInfo.TlId,
                targetObj.ObjId, attachPoint, reason);
        }
        else
        {
            _logger.Warn("UnMountMate. No valid seat entry, mountTlId: {0}, characterObjId: {1}, attachPoint: {2}, reason: {3}", mateInfo.TlId,
    ...
    public void AddActiveMateAndSpawn(Character owner, Mate mate, Item item)
    {
        ...
        _logger.Debug("Mount spawned. ownerObjId: {0}, tlId: {1}, mateObjId: {2}", owner.ObjId, mate.TlId, mate.ObjId);
    }
    ...
    public void RemoveActiveMateAndDespawn(Character owner, uint tlId)
    {
        ...
        _logger.Debug("Mount removed. ownerObjId: {0}, tlId: {1}, mateObjId: {2}", owner.ObjId, mateInfo.TlId, mateInfo.ObjId);
    }
