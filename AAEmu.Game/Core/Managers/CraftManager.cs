                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                    {
                        while (reader.Read())
                        {
                            ...
                            template.ToolId = reader.GetInt32("tool_id", 0); 
                            template.SkillId = reader.GetInt32("skill_id", 0);
                            template.WiId = reader.GetInt32("wi_id");
                            template.MilestoneId = reader.GetInt32("milestone_id", 0);
                            template.ReqDoodadId = reader.GetInt32("req_doodad_id", 0);
                            template.NeedBind = reader.GetBoolean("need_bind"); 
                            template.AcId = reader.GetInt32("ac_id", 0);
