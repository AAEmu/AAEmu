            ...
            using (var connection = SQLite.CreateConnection())
            {
                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    using (var reader = new SQLiteWrapperReader(sqliteReader))
                    {
                        while (reader.Read())
                        {
                            var holdableId = reader.GetUInt32("holdable_id", 0);
                            var template = new WeaponTemplate();
                            template.Id = reader.GetUInt32("item_id", 0);
                            ...
                            template.DurabilityMultiplier = reader.GetInt32("durability_multiplier", 0);
                ...
                using(var command = connection.CreateCommand())
                {
                    ...
                    using(var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                    {
                        while(reader.Read())
                        {
                            ...
                            //armorGradeBuff.ArmorType = (ArmorType) reader.GetUInt32("armor_type_id");
                            //armorGradeBuff.ItemGrade = (ItemGrade) reader.GetUInt32("item_grade_id");
