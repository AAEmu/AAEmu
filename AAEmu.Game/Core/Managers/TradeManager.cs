...
public class TradeManager : Singleton<TradeManager>
{
    ...
    private void UnlockTrade(Character owner, Character target, uint tradeId)
    {
        ...
        _logger.Info("Trade Id:{0} Lockers opened and Ok undone.", tradeId);
    }
    ...
    public void CanStartTrade(Character owner, Character target)
    {
        ...
        _logger.Info("{0}({1}) is trying to trade with {2}({3}).", owner.Name, owner.ObjId, target.Name, target.ObjId);
    ...
    public void StartTrade(Character owner, Character target)
    {
        ...
        _logger.Info("Trade Id:{4} started between {0}({1}) - {2}({3}).", owner.Name, owner.ObjId, target.Name, target.ObjId, nextId);
    ...
    public void CancelTrade(uint objId, int reason, uint tradeId = 0u)
    {
        ...
        _logger.Info("Trade Id:{4} between {0}({1}) - {2}({3}) is canceled.", owner.Name, owner.ObjId, target.Name, target.ObjId, tradeId);
    ...
    public void AddItem(Character character, SlotType slotType, byte slot, int amount)
    {
        ...
        if (tradeId != 0 && item.Count >= amount)
        {
            ...
            if (isOwnerWhoAdd)
            {
                _logger.Info("Trade Id:{0} {1}({2}) added item ({3}-{4}) Amount: {5}.", tradeId, owner.Name, owner.ObjId, slotType, slot, amount);
            ...
            {
                _logger.Info("Trade Id:{0} {1}({2}) added item ({3}-{4}) Amount: {5}.", tradeId, target.Name, target.ObjId, slotType, slot, amount);
    ...
    public void AddMoney(Character character, int moneyAmount)
    {
        ...
        if (tradeId != 0 && character.Money >= moneyAmount)
        {
            ...
            if (isOwnerWhoAdd)
            {
                _logger.Info("Trade Id:{0} {1}({2}) changed Money: {3}.", tradeId, owner.Name, owner.ObjId, moneyAmount);
            ...
            {
                _logger.Info("Trade Id:{0} {1}({2}) changed Money: {3}.", tradeId, target.Name, target.ObjId, moneyAmount);
    ...
    public void RemoveItem(Character character, SlotType slotType, byte slot)
    {
        ...
        if (tradeId != 0 && item != null)
        {
            ...
            if (isOwnerWhoAdd)
            {
                _logger.Info("Trade Id:{0} {1}({2}) tookdown item ({3}-{4}).", tradeId, owner.Name, owner.ObjId, slotType, slot);
            ...
            {
                _logger.Info("Trade Id:{0} {1}({2}) tookdown item ({3}-{4}).", tradeId, target.Name, target.ObjId, slotType, slot);
    ...
    public void LockTrade(Character character, bool _lock)
    {
        ...
        if (tradeId != 0)
        {
            ...
            if (!_lock)
            {
                _trades[tradeId].LockOwner = false;
                _trades[tradeId].LockTarget = false;
                _logger.Info("Trade Id:{0} {1}({2}) - {3}({4}) unlocked trade.", tradeId, owner.Name, owner.ObjId, target.Name, target.ObjId);
            }
            else if (isOwnerWhoAdd)
            {

                _trades[tradeId].LockOwner = true;
                _logger.Info("Trade Id:{0} {1}({2}) locked trade.", tradeId, owner.Name, owner.ObjId);
            }
            else
            {
                _trades[tradeId].LockTarget = true;
                _logger.Info("Trade Id:{0} {1}({2}) locked trade.", tradeId, target.Name, target.ObjId);
            }
    ...
    public void OkTrade(Character character)
    {
        ...
        if (tradeId != 0)
        {
            ...
            if (isOwnerWhoAdd)
            {
                ...
                _logger.Info("Trade Id:{0} {1}({2}) ok trade.", tradeId, owner.Name, owner.ObjId);
            }
            else
            {
                _trades[tradeId].OkTarget = true;
                _logger.Info("Trade Id:{0} {1}({2}) ok trade.", tradeId, target.Name, target.ObjId);
            }
    ...
    public void FinishTrade(Character owner, Character target, uint tradeId)
    {
        ...
        _logger.Info("Trade Id:{0} finished. Owner Items/Money: {1}/{2}. Target Items/Money: {3}/{4}",
