...
public class CSExpeditionApplicantAcceptPacket() : GamePacket(CSOffsets.CSExpeditionApplicantAcceptPacket, 1)
{
    ...
    public override void Read(PacketStream stream)
    {
        ...
        foreach (var type in Types.Distinct())
        {
            ...
            var id = (uint)type;
            if (service.Accept(Connection.ActiveChar, id) != ExpeditionRecruitmentResult.Success) continue;
            Connection.ActiveChar.SendPacket(new SCExpeditionApplicantAcceptPacket(type));
            WorldManager.Instance.GetCharacterById(id)?.SendPacket(new SCExpeditionApplicantResultPacket(true,
                expeditionName));
        }
    }
}
