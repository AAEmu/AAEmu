...
public class PingPacket() : GamePacket(PPOffsets.PingPacket, 2)
{
    ...
    /// <summary>
    /// Server-initiated keepalive ping. The reference server sends X2::PingPacket (type 0x12, level 2)
    /// on a periodic timer. The 10.0.2.13 client's game-channel recv watchdog ("recv exception:
    /// internal 4 wsa 258" = WAIT_TIMEOUT) drops conn 1239 ~2s after the char-list unless it keeps
    /// receiving server data. Level 2 bypasses the encryption gate (the level-2 dispatch path has no
    /// gate, unlike level 1, which rejects plain non-{153,154,387} types as "encryption-mv"), so a
    /// plain ping is always accepted and resets the watchdog. Body matches the client Ping
    /// deserializer: tm(i64) + when(i64) + local(u32) = 20 bytes.
    /// </summary>
