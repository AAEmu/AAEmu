...
#pragma warning restore CA1508 // Avoid dead conditional code
    ...
    // 10.0.2.13 lobby character serialization — byte-exact mirror of the client deserializer sub_39A40210.
    // Differs from the 1.2 Write above: heirExp(i64) after level; abilities BEFORE equipment; equipment =
    // validFlags(u64) + per-set-bit item structs + a SECOND "flags" u64 (sub_39A3F7A0); appearance = ext(byte)
    // + (if ext!=0) full CustomModel; then autoUseAApoint(u8) + guid(string,16) + labor block (sub_39A3B010,40B).
    ...
    public PacketStream WriteLobby1013(PacketStream stream)
    {
        ...
        // x2game-dev_dedicate.dll sub_39B165E0 (per-character writer; 8000-byte struct) invoked from
        // CharacterListPacket::SerializeBody sub_39C18280; field names/order/types follow its ISerialize calls.
        ...
        // equipment (LobbyChar_WriteEquipment 0x3938AC90) comes BEFORE abilities: validFlags(u64) + occupied
        ...
        // position record (sub +48/"pos"/+56 framing carries no extra bytes in the binary archive)
        ...
        // appearance (LobbyChar_WriteAppearance 0x393881E0): ext-gated block carrying race/gender + customization
