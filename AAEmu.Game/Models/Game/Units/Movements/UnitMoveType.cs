...
namespace AAEmu.Game.Models.Game.Units.Movements
{
    ...
    public class UnitMoveType : MoveType
    {
        ...
        public override void Read(PacketStream stream)
        {
            ...
            ActorFlags = (ActorMoveType)stream.ReadUInt16();
            ...
            if (((ushort)ActorFlags & 0x20) == 0x20)
            {
                ...
                GcPart = stream.ReadUInt16();          // actor.gcPart add in 3.5.0.3
            ...
            if (((ushort)ActorFlags & 0x100) == 0x100)
                MaxPushedUnitId = stream.ReadUInt32(); // actor.maxPushedUnitId in 3.5.0.3
        }
        ...
        public override PacketStream Write(PacketStream stream)
        {
            ...
            if ((Flags & 0x80) == 0x80)
                stream.Write(FallVel);   // actor.fallVel
            if ((Flags & 0x20) == 0x20)
            {
                stream.Write(GcFlags);   // actor.gcFlags
                stream.Write(GcPart);    // actor.gcPart add in 3.5.0.3
                stream.Write(GcPartId);  // actor.gcPartId
            ...
            if (((ushort)ActorFlags & 0x60) != 0)
                stream.Write(GcId);      // actor.gcId
            if (((ushort)ActorFlags & 0x40) == 0x40)
                stream.Write(ClimbData); // actor.climbData
            if (((ushort)ActorFlags & 0x100) == 0x100)
                stream.Write(MaxPushedUnitId);   // actor.maxPushedUnitId in 3.5.0.3
