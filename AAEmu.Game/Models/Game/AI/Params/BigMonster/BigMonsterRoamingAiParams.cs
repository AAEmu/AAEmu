...
{
    public class BigMonsterRoamingAiParams : AiParams
    {
        ...
        public List<AiCombatSkill> CombatSkills { get; set; }
        ...
        public override void Parse(string data)
        {
            using (var aiParams = new AiLua())
            {
                ...
                CombatSkills = new List<AiCombatSkill>();
                if (aiParams.GetTable("data.combatSkills") is LuaTable table)
                {
                    foreach (var skillList in table.Values)
                    {
                        ...
                        {
                            var combatSkill = new AiCombatSkill();
