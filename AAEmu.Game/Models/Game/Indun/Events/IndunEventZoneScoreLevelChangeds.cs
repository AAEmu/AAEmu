...
/// 시에만)", "3레벨로 변경 시 (레벨 상승 시에만)"). The server tracks no zone scores
/// (<c>zone_score_kinds</c> / <c>zone_score_levels</c> are unloaded and SCZoneScoreUpdatePacket has no
/// sender), so the event loads and stays inert.
/// </summary>
