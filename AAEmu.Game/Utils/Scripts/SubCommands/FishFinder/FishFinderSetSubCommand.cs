...
namespace AAEmu.Game.Utils.Scripts.SubCommands.FishFinder
{
    public class FishFinderSetSubCommand : SubCommandBase
    {
        ...
        public override void Execute(ICharacter character, string triggerArgument, IDictionary<string, ParameterValue> parameters)
        {
            ...
            if (start == "true")
            {
                FishSchoolManager.Instance.FishFinderStart((Character)character);
                SendMessage(character, $"FishFinder set start: true");
            }
            else
            {
                FishSchoolManager.Instance.StopFishFinderTickAsync((Character)character).GetAwaiter().GetResult();
