...
public class CharacterManagerTests
{
    ...
    #region DeleteCharacterAssets Tests

    // Note: DeleteCharacterAssets uses static singletons (ExpeditionManager, mailManager.AllPlayerMails)
    // which require complex setup for unit testing. These tests would need integration testing setup.

    [Fact(Skip = "Requires static singleton setup - method uses ExpeditionManager and mailManager.AllPlayerMails")]
    public void DeleteCharacterAssets_NotFullWipe_DoesNotDeleteItems()
    {
        // Arrange
        var mockHousingManager = new Mock<IHousingManager>();
        var mockFamilyManager = new Mock<IFamilyManager>();
        var mockMailManager = new Mock<IMailManager>();
        var manager = CreateCharacterManager(
            mockHousingManager: mockHousingManager,
            mockFamilyManager: mockFamilyManager,
            mockMailManager: mockMailManager);

        var character = new Character(new UnitCustomModelParams())
        {
            Id = 1,
            Name = "TestCharacter",
            Mails = new CharacterMails(new Character(new UnitCustomModelParams()))
        };

        // Act
        manager.DeleteCharacterAssets(character, false);
    ...
    [Fact(Skip = "Requires static singleton setup")]
    public void DeleteCharacterAssets_NullCharacter_ThrowsNullReferenceException()
    {
        // Arrange
        var manager = CreateCharacterManager();

        // Act & Assert
        Assert.Throws<NullReferenceException>(() => manager.DeleteCharacterAssets(null, false));
    }

    #endregion

    #region IsCharacterPendingDeletion Tests

    // Note: IsCharacterPendingDeletion queries the database directly,
    // making it unsuitable for unit testing without a database mock.
    // These tests would require integration testing setup with a real or mocked MySQL connection.

    [Fact(Skip = "Requires database connection - method queries MySQL directly")]
    public void IsCharacterPendingDeletion_ExistingDeletedCharacter_ReturnsTrue()
    {
        // Arrange
        var manager = CreateCharacterManager();

        // Act & Assert - This test requires database setup
        // Method queries database directly, so we can't unit test it without mocking MySQL
    }

    [Fact(Skip = "Requires database connection - method queries MySQL directly")]
    public void IsCharacterPendingDeletion_NonExistentCharacter_ReturnsFalse()
    {
        // Arrange
        var manager = CreateCharacterManager();

        // Act
        var result = manager.IsCharacterPendingDeletion("NonExistentCharacterName12345");
