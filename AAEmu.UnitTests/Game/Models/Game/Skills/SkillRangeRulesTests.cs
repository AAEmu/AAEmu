...
/// <summary>
/// The range band as x2game-dev.dll FUN_39d1e4e0 judges it: too close at or inside min_range, too far beyond
/// max_range, and no measurement when the target is the caster (FUN_39800cc0). The bands are real rows:
...
public class SkillRangeRulesTests
{
    ...
    {
        // FUN_39d1e4e0 passes only when min < distance, so the boundary itself is refused.
    ...
    {
        // The far side is inclusive: FUN_39d1e4e0 refuses only when max < distance.
    ...
    {
        // The weapon bands start at 0, and FUN_39d1e4e0 only runs the near check when min > 0.
    ...
    {
        // 46100 술통 기뢰 is the one row shipped this way (3..0); FUN_39261740 lifts it to min + 0.5.
    ...
    {
        // FUN_39800cc0 calls ValidateLocation only for a target that is not the caster, which is what lets
    ...
    public async Task AMinRangeModifier_MovesTheNearBoundary()
    {
        ...
        // minimum before the band is judged (FUN_39261740).
