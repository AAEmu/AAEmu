...
public class MateSeatCatalogTests
{
    [Test]
    public async Task OneSeat_RemovesDefaultPassengerAndKeepsDriver()
    {
    ...
    [Test]
    public async Task MultiSeat_IsOrderedAndDeduplicated()
    {
        var catalog = new MateSeatCatalog();
    ...
    [Test]
    public async Task InvalidRows_AreIgnoredInsteadOfCreatingNonSeatSlots()
    {
        var catalog = new MateSeatCatalog();
        catalog.LoadFromRows([
            (303, 0),
            (303, (int)AttachPointKind.Cannon0),
            (303, 35),
    ...
    [Test]
    public async Task Load_UsesTheNpcMountJoinAndDropsNonSeatRows()
    {
        using var connection = new SqliteConnection("Data Source=:memory:");
        connection.Open();
        Execute(connection, "CREATE TABLE npc_mount_skills (npc_id INTEGER NOT NULL, mount_skill_id INTEGER NOT NULL)");
