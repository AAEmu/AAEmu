...
public sealed class AccountReturnIntegrationTests
{
    ...
    [Fact]
    public async Task EntitlementsTierAndPayment_LoadInTheConnectionPath_AndSurviveRelog()
    {
        var supplied = Environment.GetEnvironmentVariable(EnvironmentVariable);
        Assert.SkipUnless(!string.IsNullOrWhiteSpace(supplied),
            $"Set {EnvironmentVariable} for an isolated schema.");

        var contentPath = Path.Combine(AppContext.BaseDirectory, "Data", "compact.sqlite3");
        Assert.SkipUnless(File.Exists(contentPath), $"Missing shipped content at {contentPath}.");

        var (isolated, connectionString) = await CreateIsolatedSchema(supplied);
        await Execute(isolated, AccountAttributesSchema);
        await Execute(isolated,
            $"INSERT INTO account_attributes (account_id, kind_id, kind_value, world_id, count, starts, expires) " +
            $"VALUES ({AccountId}, 2, 1001, 0, 0, '2026-01-01 00:00:00', '9999-12-31 23:59:59')");

        var previousContent = SwapContentConfig(new ContentConfigGameData());
        var previousPremium = SwapSingleton<PremiumGameData>(new PremiumGameData());
        var previousAccountManager = SwapSingleton<AccountManager>(
            new AccountManager(Mock.Of<ITickManager>(), Mock.Of<ITimedRewardsManager>()));
        try
        {
            using (var sqlite = new SqliteConnection($"Data Source={contentPath};Mode=ReadOnly"))
            {
                sqlite.Open();
                PremiumGameData.Instance.Load(sqlite);
            }

            AccountAttributeManager.Instance.Load(isolated);

            var connection = LoadConnectionPath();
            Assert.True(connection.AccountTier.Grade >= PremiumGameData.Instance.FirstPaidGradeId,
                $"tier {connection.AccountTier.Grade} must reach the content paid floor");
            Assert.Equal(PaymentMethodType.Premium, connection.Payment.Method);
            Assert.True(connection.Entitlements.Any(a =>
                    a.KindId == (uint)AccountAttributeKind.AccountBuff && a.KindValue == 1001),
                "the persisted membership must load on the connection");

            // Relog: a brand-new connection loads the same state back from the store.
            var afterRelog = LoadConnectionPath();
            Assert.Equal(connection.AccountTier, afterRelog.AccountTier);
            Assert.Equal(connection.Payment.Method, afterRelog.Payment.Method);
            Assert.True(afterRelog.Entitlements.Any(a => a.KindValue == 1001u));
        }
        finally
        {
            RestoreSingleton<AccountManager>(previousAccountManager);
            RestoreSingleton<PremiumGameData>(previousPremium);
            RestoreContentConfig(previousContent);
            await DropIsolated(supplied, isolated);
        }
    }

    /// <summary>One pass of the 10.x connection path: LoadAccount's own state load.</summary>
    private static GameConnection LoadConnectionPath()
    {
        var connection = new GameConnection(null) { AccountId = AccountId };
        connection.Characters[1u] = new Character(new UnitCustomModelParams())
        {
            Id = 1, AccountId = AccountId, Point = 400
        };
        AccountConnectionState.Load(connection);
        return connection;
    }
