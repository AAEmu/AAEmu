...
2. **Network Layer** – set of classes in `AAEmu.Game/Core/Network/*`:
...
3. **Packet System** – files in `AAEmu.Game/Core/Packets/*` (C2G, C2S, G2C, S2C, Proxy, L2G, G2L). Contains business logic for validation and handling of game actions (e.g., `CSBuyItemsPacket.cs`, `SCCharacterListPacket.cs`).
4. **Game Data** – `AAEmu.Game/GameData/*.cs`. Stores static and dynamic information about items, NPCs, locations, quests, etc. This is the core of business logic (e.g., `ItemGameData.cs`, `NpcGameData.cs`).
5. **Configuration** – `AAEmu.Game/Config.json` and files in `AAEmu.Game/Configurations/`. Contains server parameters, character settings, balance, etc.
6. **Utilities and Common Libraries** – `AAEmu.Commons/*`. Provides helper functions (XML serialization, logging, etc.).
...
2. **Add Moq**: `dotnet add package Moq`.
3. **Configure `Directory.Build.props`** (if needed) for unified build settings.
4. **Add base test fixtures** (`TestBase.cs`) with common mock objects.
5. **Implement tests sequentially by priority** (first services and network layer, then business logic data).
6. **Integrate into CI** (GitHub Actions, Azure Pipelines) – run `dotnet test` after each build.
7. **Track coverage**: add `coverlet.collector` and generate report `dotnet test /p:CollectCoverage=true`.
