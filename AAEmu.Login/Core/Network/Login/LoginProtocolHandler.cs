...
public class LoginProtocolHandler : BaseProtocolHandler
{
    ...
    {
        _logger.Debug($"Connection from {session.Ip} established, session id: {session.SessionId}");
        try
        {
    ...
    public override void OnDisconnect(Session session)
    {
        if (session is null)
        {
            _logger.Error("Unexpected null Session");
            return;
        }
        try
        {
        ...
        _logger.Debug($"Client from {session.Ip} disconnected");
    }
