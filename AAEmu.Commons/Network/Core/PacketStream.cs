...
namespace AAEmu.Commons.Network.Core
{
    ...
    public class PacketStream : ICloneable, IComparable
    {
        ...
        public T ReadObject<T>() where T : IReadable, new()
        {
            var t = new T();
            t.Read(this);
            return t;
        ...
        public PacketStream Write(IWritable writable)
        {
            writable.Write(this);
            return this;
        }
        ...
        public PacketStream WriteAsMarshaller(PacketStream value)
        {
            return value.Write(this);
        }
        ...
        #region Write Hex Strings

        public PacketStream Write(string value, bool appendSize = true, bool appendTerminator = false, bool toHex = false)
        {
            if (toHex)
            {
                value = value.Replace(" ", ""); // удалим все пробелы
                if (value.Length % 2 != 0)
                {
                    Console.Write("Network: Attempted to WriteHex() the binary key cannot have an odd number of digits\n");
                    throw new MarshalException();
                }
                var hex = new byte[value.Length / 2];
                for (var i = 0; i < hex.Length; i++)
                {
                    hex[i] = byte.Parse(value.Substring(i * 2, 2), System.Globalization.NumberStyles.HexNumber);
                }

                return Write(hex, appendSize);
            }
            else
            {
                var str = Encoding.UTF8.GetBytes(appendTerminator ? value + '\u0000' : value); // utf-8
