...
namespace AAEmu.Commons.Cryptography
{
    public class EncryptionManager : Singleton<EncryptionManager>
    {
        ...
        public void StoreClientKeys(byte[] aesKeyEncrypted, byte[] xorKeyEncrypted, ulong accountId, uint connectionId)
        {
            ...
            //head = (head ^ 0x15A023EE) * head ^ 0x70F1F23 & 0xffffffff; // 1.1.2.8 r588242 03.09.2022 ArcheWorld XLGames
            head = (head ^ 0x15A023F0) * head ^ 0x70F1F23 & 0xffffffff; // 1.1.2.9 r589719 14.09.2022 ArcheWorld XLGames
        ...
        public static byte[] DecodeXor(byte[] bodyPacket, uint xorKey, ConnectionKeychain keys)
        {
            ...
            //var cry = mul ^ ((uint)MakeSeq(keys) + 0x75A02404) ^ 0x629B62BB; // 1.1.2.8 r588242 03.09.2022 ArcheWorld XLGames
            var cry = mul ^ ((uint)MakeSeq(keys) + 0x75A02406) ^ 0xE2B74104; // 1.1.2.9 r589719 14.09.2022 ArcheWorld XLGames
            var offset = 4;
            if (seq != 0)
            {
                if (seq % 3 != 0)
